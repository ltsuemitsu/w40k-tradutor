# W40K: Rogue Trader - Tradutor GUI Launcher (PowerShell)
# Right-click → Run with PowerShell or create a shortcut to this file
# This version is more robust with external command error handling
...
Write-Host "   W40K ROGUE TRADER - TRADUTOR (GUI)" -ForegroundColor Yellow
...
Write-Host "(A janela do Tradutor vai abrir em alguns segundos...)" -ForegroundColor Gray
...
try {
    ...
    & $pythonwExe @pythonwArgs "tradutor_desktop.py"
} catch {
    Write-Host ""
    Write-Host "[AVISO] Falha ao iniciar com versao sem console (pythonw). Tentando versao normal..." -ForegroundColor Yellow
    Invoke-PySafe @("tradutor_desktop.py") | Out-Null   # just in case
    & $pythonExe @pythonArgs "tradutor_desktop.py"
}
...
if ($LASTEXITCODE -ne 0 -and $LASTEXITCODE -ne $null) {
    ...
    Write-Host "    py -3 tradutor_desktop.py" -ForegroundColor White
    Write-Host "================================================" -ForegroundColor Red
